...
export class SeriesComponent {
  ...
  getRowCards(rowIndex: number): SeriesCard[] {
    const startIndex = rowIndex * 3;
    return this.seriesCards.slice(startIndex, startIndex + 3);
  }
  ...
  deleteCard(cardId: number): void {
    const card = this.seriesCards.find(c => c.id === cardId);
    if (card) {
      card.isDeleting = true;
      setTimeout(() => {
        this.seriesCards = this.seriesCards.filter(c => c.id !== cardId);
      }, 2000);
    }
  }
}